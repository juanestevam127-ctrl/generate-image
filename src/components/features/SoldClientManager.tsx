...
export function SoldClientManager() {
    const { soldClients, addSoldClient, updateSoldClient, deleteSoldClient } = useStore();
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error("Save Sold Client Error:", error);
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-blue-400">
                    Gerenciamento de Clientes (Imagens Vendidos)
                </h2>
                <Button onClick={openNewClientModal} className="bg-green-600 hover:bg-green-700 text-slate-950 font-bold">
                    <Plus className="w-4 h-4 mr-2" /> Novo Cliente
                </Button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {[...soldClients].sort((a, b) => a.name.localeCompare(b.name)).map((client) => (
                            ...
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <Button variant="ghost" size="icon" className="h-8 w-8 hover:text-red-400" onClick={() => {
                                        if (confirm("Excluir cliente?")) deleteSoldClient(client.id);
                                    }}>
                                        <Trash size={16} />
                                    </Button>
                                </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                                <Button
                                    variant="outline"
                                    className="flex-1 relative h-10 border-dashed border-green-500/30 hover:border-green-500/60"
                                    disabled={isUploading.stories}
                                    onClick={() => document.getElementById('upload-sold-guide-stories')?.click()}
                                >
                                ...
                                <input
                                    type="file"
                                    id="upload-sold-guide-stories"
                                    className="hidden"
                                    accept="image/*"
                                    onChange={(e) => handleFileUpload(e, 'stories')}
                                />
                                {guideStories && (
                                    <div className="w-10 h-10 rounded border border-green-500/20 overflow-hidden cursor-pointer" onClick={() => window.open(guideStories, '_blank')}>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center gap-2">
                                <Button
                                    ...
                                    onClick={() => document.getElementById('upload-sold-guide-feed')?.click()}
                                >
                                ...
                                <input
                                    type="file"
                                    id="upload-sold-guide-feed"
                                    className="hidden"
                                    accept="image/*"
                                    onChange={(e) => handleFileUpload(e, 'feed')}
                                />
                                {guideFeed && (
                                    <div className="w-10 h-10 rounded border border-green-500/20 overflow-hidden cursor-pointer" onClick={() => window.open(guideFeed, '_blank')}>
